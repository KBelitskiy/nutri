...
- Команда запуска: `python3 -m bot.main`.
- Для продакшена рекомендуется использовать managed PostgreSQL (`DATABASE_URL`), так как SQLite может быть неперсистентной между деплоями.
